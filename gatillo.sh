...
# Obtener la IP pública del servidor en AWS
SERVER_IP=$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4)

if [ -z "$SERVER_IP" ]; then
    echo "No se pudo detectar la IP pública del servidor en AWS."
    exit 1
fi

echo -e "\n\033[1;32mLa IP pública del servidor es:\033[0m $SERVER_IP"
